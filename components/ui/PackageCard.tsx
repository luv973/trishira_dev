...
export function PackageCard({ pkg }: { pkg: ConstructionPackage }) {
  ...
  return (
    <article
      className={cn(
        "relative flex h-full flex-col rounded-3xl border p-6 transition-all duration-300 hover:-translate-y-2 sm:p-7",
        dark
    ...
    >
      {pkg.popular && (
        <span className="absolute -top-3.5 left-1/2 inline-flex -translate-x-1/2 items-center gap-1.5 rounded-full bg-linear-to-r from-gold-300 to-gold px-4 py-1.5 text-[0.66rem] font-bold uppercase tracking-[0.16em] text-navy shadow-gold whitespace-nowrap">
      ...
      <div className="flex items-center justify-between gap-3">
        <span
          className={cn(
            "rounded-full px-3 py-1 text-[0.66rem] font-bold uppercase tracking-[0.14em]",
            theme.badge,
          )}
        >
          {pkg.badge}
      ...
      <h3 className={cn("mt-4 text-xl sm:text-[1.35rem]", dark ? "text-cream" : "text-navy")}>
        {pkg.name}
      </h3>
      ...
      <div className="mt-5 flex items-end gap-1.5">
        <span className={cn("text-3xl font-extrabold tracking-tight sm:text-4xl", dark ? "text-gold" : "text-navy")}>
          ₹{pkg.priceLabel}
        </span>
      ...
      <ul className="mt-5 flex-1 space-y-2.5">
        {pkg.scope.slice(0, 5).map((item) => (
          ...
          >
            <Check
              className={cn("mt-0.5 h-4 w-4 shrink-0", "text-gold")}
              strokeWidth={2.6}
            />
            {item}
          </li>
      ...
      <Link
        ...
        className={cn(
          "group mt-6 inline-flex items-center justify-center gap-2 rounded-full px-5 py-3 text-sm font-semibold transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold focus-visible:ring-offset-2",
          dark
